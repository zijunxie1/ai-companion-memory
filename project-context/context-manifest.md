...
**启动回执保持紧凑，不挤占 Founder 正文**：启动回执是强制核验，不得省略或削弱；但当回执对 Founder 可见时保持紧凑，只呈现恢复身份和批准边界所必需的信息（当前角色、统一规则版本 C1、核验后的正式主线提交 C3、上下文来源声明 C6、当前交互阶段、允许/禁止、结论）。完整 Git、文件、命令、核验细节进入任务交接文件或实施报告，不进 Founder 正文；`## 先说人话` 永远先于启动回执出现，回执不得挤占它。
