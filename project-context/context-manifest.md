...
7. 重新判断当前交互阶段：讨论 / 已决定 / 交接或执行；特别区分“当前阶段已完成”和“下一阶段已批准”；
8. 重做 Git 只读核验（分支 / HEAD / Worktree / 工作区）；
9. 按 `agent-response-protocol.md` 重新选择 L1 / L2 / L3 密度，并先用大白话说明当前工作；
10. 核对下游执行需要的完整判断是否已在交接文件中；
11. 无冲突后继续；若已有唯一明确的 Founder 决定，直接恢复执行，不得要求二次确认。

> **聊天摘要、Memory、全局 Skill 和旧会话只能用于定位线索，不能作为任务批准、Git 状态、正式决策或回复格式的唯一证据。** 若其中写“Founder 已同意”但任务/交接/决策文件没有准确记录批准边界，必须停止并补证；若正式文件已经准确记录，则不得因压缩重新让 Founder 确认一遍。全局 Skill 与仓库正式规则冲突时，以重新读取的仓库规则为准；不得照搬旧会话的最终回答。
