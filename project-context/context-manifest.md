...
新窗口、新任务、角色交接、上下文压缩后恢复或仓库状态变化后，Agent 的第一份实质性输出必须先以 `## 先说人话（30 秒）` 开头，再给一份**Founder 可见的紧凑启动回执**：
...
回执最多七个信息行，不新增“已阅读、分支、Worktree、完整 Git、Chief 类型、升级路由、历史归因”等独立行。新任或新窗口 Chief 的实例、关系和类型合并在“角色与关系”一行；升级路由、继任核对和历史归因只在发生异常或需要 Founder 判断时进入正文，否则写入任务/交接内部记录。
...
启动回执不是形式证明。Agent 必须先做只读 Git/文件核对，不能只根据聊天摘要填写；但核验完整不等于聊天必须展示完整。`## 先说人话` 永远先于回执出现，回执不得重复正文的建议或当前决定。
