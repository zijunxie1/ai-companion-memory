...
| D-T006-LOCAL-SPIKE | TASK-006 本地相关性 Gate Spike（候选机制、限时、冻结 holdout、延迟/资源门、验收、停止条件、允许/禁止范围） | **APPROVED（Spike = STOPPED/FAILED；已裁决选项 A；收尾 PR 待合并）** | Founder 2026-08-12 批准 DRAFT v1.2（任务方案 Review 通过；两轮 CHANGES_REQUESTED 修订后）：候选两机制（候选 1 本地词法/统计二次相关性 Gate 含非冗余诊断 ρ<0.9 + 候选 2 本地主题类别 Gate 可执行定义）；至少一个正式候选通过 §8 全部门即形成"存在可行候选"结论，双候选失败才停止；限时 5 工作日/3 轮；四步冻结顺序（holdout 数据冻结 → 校准调参 → 机制冻结 → holdout 一次性运行）；延迟增量 P95 ≤ 200ms / 总预算 ≤ 1000ms（处理组−基线）；资源 ≤ 512MB；校准+holdout 双 F1 ≥ 0.9 且分离边际 > 0.1；合成数据 + loopback-only + 预装检查；证据落盘 `TASK-006/spike/`。**执行模式已确认**：HANDOFF REQUIRED / persistent_session（Founder 2026-08-12）；草案分支推送单一规划 PR（仅 DRAFT/decision-register/current-state 三文件，不含产品代码）；规划 PR 合入主线并只读核验成功后，才从最新 origin/main 创建实施分支并唤醒长期 Builder。**Spike 执行事实（2026-08-12）**：实施计划 v1.0 经 Review 2 批准后执行阶段 2；候选 1 校准失败（ρ=0.8653 非冗余但 16 版本边际全负）；候选 2 校准通过（F1=1.0/边际 0.3971）但 holdout 一次性运行失败（H1-H3 PASS、H4 天气变体 FAIL，合并 F1=0.8571/边际 0.0013）→ **两个正式候选都失败，DRAFT v1.2 §9 停止条件 9 触发，Spike 停止（STOPPED/FAILED）**。未形成"存在可行候选"结论；E004 缺陷仍存在；**不等于 TASK-006 失败或完成（TASK-006 保持 APPROVED）**。**Founder 2026-08-12 裁决（CR-T006-SPIKE-STOP-01）= 选项 A**：接受失败结论；禁止补 H4 词表、重跑冻结 holdout、重启外部模型路线。收尾：推送本地证据 + 停止报告 + 裁决记录，创建单一收尾 PR（状态 STOPPED/FAILED）交独立 Reviewer 核对；**收尾 PR 合并后**再由执行 Chief 起草第二轮候选范围 DRAFT（针对词法不可分 + 手工词表覆盖不足，不预先选定方案）；新 DRAFT 批准前不唤醒 Builder、不修改产品/评测规则 | TASK-006 | `tasks/TASK-006/local-gate-spike-draft.md`（v1.2 + §11.1）、`tasks/TASK-006/spike-stop-cr.md`（DECIDED-A）、`tasks/TASK-006/spike/`（冻结记录/原始数据/审计）、`tasks/TASK-006/draft.md`、`tasks/TASK-006/route-b-decision.md`、`current-state.md` |
