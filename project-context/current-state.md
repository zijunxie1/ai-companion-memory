...
>
> 最近只读核验：2026-08-15（Asia/Shanghai）；核验人：GOV-COMM-003 治理 Builder 窗口
> 核验方式：`git fetch origin main` / `git ls-remote` / `git rev-parse` / `git log` / `git worktree list` / `git status`；正式主线治理文件与 GOV-COMM-002 合并事实；未查询数据库、未调用外部模型。
> 本快照以本次核验时的 `origin/main @ df5c2d9` 为基础；GOV-COMM-002（PR #26）已 Rebase 合并，Founder 对话/Agent 交接分层规则已成为正式主线行为；TASK-006 第三轮收尾 PR #25 已合并。GOV-COMM-003 的自适应回复与长方案审批翻译正在独立治理分支实施，未合并前不属于正式主线行为。每次读取仍须重新核验远端主线，快照不会自动更新。
>
> 更新要求：重要状态变化、合并、部署、角色交接或上下文恢复后更新；不得提前写入未发生状态。

> 非自动更新：本文件只有在某个角色被 Founder 唤醒并实际写入时才会变化。每次读取必须重新用 Git 和任务文件核对。

> 上下文规则版本：`2026-08-15.4`（本治理变更 GOV-COMM-003；未合并前正式主线仍为上一版本 `2026-08-15.3`，读取时以远端 main 的 `AGENTS.md` 为准）

## 一句话状态

P1 已形成真实聊天、Memory、Trace 和 8 Case Eval 纵向闭环，E004 无关召回问题仍未解决，TASK-006 保持 `APPROVED`。第三轮 A/B/C 全部 STOPPED/FAILED，holdout 10 对零读取；R3 收尾 PR #25 已合并并保存失败证据。Founder 仅批准 R4 方向与起草，尚未批准产品实现或新实验。GOV-COMM-002（PR #26）已合并：Founder 大白话讨论、Agent 完整交接、明确决定后直接执行和压缩恢复边界已固化为统一规则。GOV-COMM-003 正在补入「复杂方案审批必须先完成决策翻译」与「密度指导非机械格式」，让回复密度随事情复杂度自适应而非固定模板；未合并前不属于正式主线行为。项目未 CLOSED，未进行生产部署。
...
| `origin/main`（本次核验快照） | `df5c2d9`（GOV-COMM-002 PR #26 已 Rebase 合并；含 Founder 对话/Agent 交接分层、密度自适应、明确决定不二次确认、压缩恢复边界及 Skill 污染清理；读取时必须重新核验最新 tip） |
...
| 当前治理 Worktree | `E:/gov-founder-adaptive-response-worktree`（`codex/gov-founder-adaptive-response`，从 `origin/main @ df5c2d9` 创建；只修改协作治理文件） |
...
| GOV-COMM-003 | **IMPLEMENTED（待独立 Review；未合并前不属于正式主线事实）** | Founder 2026-08-15 已批准先修正 Hermes 回复规范（自适应密度 + 复杂方案审批决策翻译）再恢复 TASK-006 R4。补入：回复密度「句数/章节数是指导非机械格式」；复杂方案审批必须先完成决策翻译（处境/方案/能不能解决/好处代价/风险未知/推荐理由/当前决定 + 禁止审批话术）；角色回复内容重点；启动回执紧凑不挤占 Founder 正文。实施仅在 `codex/gov-founder-adaptive-response`，未合并前不属于正式主线规则 |
...
6. **GOV-COMM-003 已实施（待独立 Review）**：Founder 批准先修正 Hermes 回复规范再恢复 TASK-006 R4。已补入「复杂方案审批必须先完成决策翻译」与「密度指导非机械格式」；实施分支 `codex/gov-founder-adaptive-response`，未合并前不属于正式主线规则；
...
- **本次只需要 Reviewer 完成**：对照 `tasks/GOV-COMM-003/validation-scenarios.md` 的 A—H 八类场景，核验「复杂方案审批必须先完成决策翻译」与「密度指导非机械格式」是否落盘且不建立与 L1/L2/L3 冲突的第二套分级；确认简单回复未过度展开、复杂回复未过度压缩、旧规则和冲突表述清零。
- **不得执行**：Reviewer 不修改分支文件、不合并、不启动 R4、不读取 holdout；允许且要求向对应 PR 提交 Review / Comment 作为完整报告。
- **必须阅读**：`tasks/GOV-COMM-003/draft.md`、`tasks/GOV-COMM-003/validation-scenarios.md`、`tasks/GOV-COMM-003/implementation-report.md`、`tasks/GOV-COMM-003/reviewer-handoff.md` 及其 required_reading。
