...
> 上下文规则版本：`2026-08-15.1`（本治理变更；未合并前正式主线仍为上一版本，读取时以远端 main 的 `AGENTS.md` 为准）
...
| GOV-COMM-002 | **IMPLEMENTED（待独立 Review）** | Founder 已批准并授权直接执行 Founder 对话/Agent 交接分层、密度自适应、明确决定不二次确认、压缩恢复批准边界；实施仅在 `codex/gov-founder-alignment`，未合并前不属于正式主线规则 |
...
- **本次只需要 Reviewer 完成**：审查 Founder 对话/Agent 交接分层规则是否一致、压缩恢复是否保留批准边界、短卡/完整交接是否不再冲突，以及是否零产品/评测/TASK-006 实验改动。
- **不得执行**：Reviewer 不修改文件、不合并、不启动 R4、不读取 holdout。
