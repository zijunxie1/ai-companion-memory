...
>
> 最近只读核验：2026-08-15（Asia/Shanghai）；核验人：治理 Builder（TASK-006 第三轮 Spike 收尾）＋ 治理同步
> 核验方式：本地 `git fetch origin main` / `git rev-parse` / `git log` / `git worktree list` / `git status`；正式任务文件与执行分支证据；未查询数据库、未调用外部模型。
> 本快照以本次核验时的 `origin/main @ ccba980` 为基础；GOV-CHIEF-001、GOV-COMM-001、GOV-002（含合并后修正 PR #15）、TASK-006 第一/二轮 Spike、第三轮规划（PR #19）、第三轮治理落盘（D-T006-R3-EXEC / -B-MODEL / -C-EXT）、作品集展示阶段数据外发临时例外（D-T006-SHOWCASE-EXCEPTION，PR #22）、PR #21（R3 治理状态同步）与方案 C 校准阶段运行批准（ccba980）均已合入。每次读取仍须重新核验远端主线，快照不会自动更新。
>
...
| `origin/main`（本次核验快照） | `ccba980`（含第三轮治理落盘 D-T006-R3-EXEC / -B-MODEL / -C-EXT、展示阶段例外 D-T006-SHOWCASE-EXCEPTION PR #22、R3 治理状态同步 PR #21、方案 C 校准阶段运行批准，2026-08-13/14；含 GOV-002 合并后修正 PR #15 `5de2714`、TASK-006 第一/二轮 Spike；读取时必须重新核验最新 tip） |
...
| 其他 Worktree | `E:/gov-001-worktree` 干净；`E:/gov-001b-worktree` 干净；`E:/gov-001c-worktree` 有历史治理改动；`E:/task-005a-worktree` 干净；`E:/task-006-r3-spike-worktree`（执行分支 `feature/task-006-r3-spike`，含 S2/S3 + 方案 C 校准证据）；`E:/task-006-r3-closeout-worktree`（本收尾分支，跟踪已修正为同名远端分支 `origin/feature/task-006-r3-closeout`）；`E:/task-006-showcase-worktree`（展示阶段例外分支 `feature/task-006-showcase-exception`，已合入 main）；全部保持不动 |
...
| TASK-006 | **APPROVED** | DRAFT v1.1 已入库；E004 缺陷仍存在。Founder 已批准路线 B（不外发用户数据的本地/规则/检索路线）；**2026-08-14 展示阶段临时例外（D-T006-SHOWCASE-EXCEPTION）**允许外部模型处理 Founder 自有真实数据，未来产品化改回本地。临时计划 v1.4 与 CR-01 v1.2 均未批准；没有产品代码、实施 Worktree；**已有原实验分支证据独立 Review 报告（`spike-r3-review-report.md`，REVIEW_APPROVED 仅限 `c3d73cc` 证据；收尾分支待 PR 后独立审查）**；任务不进入 IN_PROGRESS |
...
- **本次只需要 Founder 完成**：审阅收尾分支落盘的 4 个文件（current-state / decision-register / spike-r3-builder-handoff / spike-r3-review-report）+ 21 份从 `c3d73cc` 复制并逐字节核验一致的证据文件，确认无误后授权下一步。
