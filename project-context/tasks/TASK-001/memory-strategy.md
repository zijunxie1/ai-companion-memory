...
| 对后续互动有长期价值 | 跨会话有效 | 用户养猫、稳定偏好 |
| 多次出现 | 同一信息 ≥ 2 次 | 多次提到深夜失眠 |
| 属于稳定偏好 | 不轻易变化 | 互动风格、聊天节奏 |
| 影响互动方式 | 改变 AI 应该怎么说 | 不喜欢爹味 |
...
|---|---|---|---|---|
| stable_preference | 高 | 长期，高置信度 | 不喜欢被追问原因 | ✅ M001 |
| important_event | 高 | 长期，到期后降权 | 最近刚分手 | ✅ M002 |
| relationship_node | 高 | 长期，不可自动清理 | 称呼、关系阶段 | ✅ 可合入 M004 |
| shared_experience | 高 | 长期，构成关系基础 | 养了橘猫，一起深夜聊过猫 | ✅ M004 |
| interaction_style | 高 | 长期 | 喜欢深夜闲聊，不喜欢爹味 | ✅ M003 |
| long_term_interest | 中 | 长期，召回优先级中 | 喜欢猫 | ⬜ V2 |
| transient_state | 低 | 会话级，不跨会话 | 今晚又失眠了 | ⬜ 抽取节点展示拒绝写入 |
...
2. 关键词匹配当前输入：
   - "失眠" / "睡不着" / "猫" → 返回 M004（shared_experience：养猫+深夜失眠）+ M002（important_event：刚分手）
   - "烦" / "不想聊" → 返回 M001（stable_preference：不喜欢被追问原因）
...
| 类型 | 示例 | 检测方式 |
|---|---|---|
| 直接否定 | 用户说"我不是不喜欢你问" | LLM 判断 + 关键词匹配 |
| 值变更 | 用户纠正"我现在不怕聊分手了" | 同字段不同值 |
| 语义矛盾 | 旧 Memory"情绪恢复中"，新信息"又开始崩溃了" | LLM 语义对比 |
...
V1 用 Path B 演示：
- 用户说"我不是不喜欢你问，我只是不想每次都解释为什么"
- 节点 7 检测到与 M001（不喜欢被追问原因）冲突
- 旧 M001 内容修正为更精确的表述
- 后续对话不再误用旧 Memory
...
| 级别 | 操作 | 示例 |
|---|---|---|
| 用户主动删除 | 硬删除，立即生效 | 用户在 Memory 管理页删除"最近刚分手" |
| 自动过期 | important_event 到期后降权，不删除 | 情绪恢复后"刚分手"事件衰减 |
