...
assigned_session: TASK-002｜Builder｜V2 真实 Memory 闭环
...
- ❌ 不做 20 条 Eval Case
- ❌ 不做高并发或生产级
- ❌ 不做云端部署（先本地开发，有面试再迁移火山云）

## 部署前置原则

> 全部服务最终部署到火山云轻量服务器（2核8G）。开发期全本地，但从一开始就做好迁移准备：
> 1. 所有服务用 Docker 容器化
> 2. 所有配置走环境变量（.env），不硬编码 localhost 或端口
> 3. Docker Compose 文件随时可从本地切换到云端
> 4. 数据库用 migration 脚本管理，不手动建表
> 5. 不依赖本地文件系统存储（Memory 和业务数据都在数据库中）
...
| 部署 | 火山云轻量服务器（2核8G） | 一台服务器部署全部作品（P1-P4 + 个人网站） |
