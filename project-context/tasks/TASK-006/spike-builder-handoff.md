...
按 `local-gate-spike-draft.md`（v1.2，已批准）执行 TASK-006 本地相关性 Gate Spike：验证**最多两个本地候选机制**（候选 1 = 本地词法/统计二次相关性 Gate；候选 2 = 本地主题类别 Gate）能否可靠分离 E004 场景的无关召回与 E001 等正向召回，产出可复现证据与 Spike 报告。**Spike 通过只代表"技术可行"，不代表 TASK-006 完成；不接入产品路径。**
...
- 本地组件：mem0-server、PostgreSQL（ai_companion）、Node 现有依赖、fastembed 模型缓存（BAAI/bge-small-zh-v1.5）均为已预装本地服务（loopback）；
...
未完成（Builder 全部工作）：实施计划 → 预装检查 → 校准测量（≥3 轮）→ 冻结 holdout（四步顺序）→ 延迟/资源测量 → Spike 报告与证据落盘。
...
1. 在 Spike 分支新增独立只读测量/验证脚本：评测专用 user_id、真实本地 mem0 检索（loopback）、本地词法/统计特征计算；不调用 mem0.add（冻结 holdout 种子记忆除外，测量后清理）；不落库正式表、不进入 /api/chat；
2. 读取正式代码与契约作为只读输入；
3. 证据落盘：脚本 + 原始数据 + 报告 + 冻结记录（`holdout-freeze.md` / `mechanism-freeze.md`，含时间戳/提交号/哈希）写入 `project-context/tasks/TASK-006/spike/`；
4. 使用已预装本地组件（mem0-server / PostgreSQL / Node 现有依赖 / fastembed 缓存）；
...
2. **实施计划（Review 2）**：先提交实施计划（测量脚本设计、候选评估顺序、预装检查命令、冻结记录文件结构），等待 Founder/Reviewer 批准后再执行测量；
3. **预装检查**：按 DRAFT §5.4 核验清单逐项记录到 `spike/preflight-check.md`；缺失即停止，提交范围/依赖裁决；
...
1. 实施计划（Review 2 前置）；2. `spike/preflight-check.md`；3. `spike/holdout-freeze.md` / `spike/mechanism-freeze.md`（含时间戳/提交号/哈希）；4. 测量脚本 + 原始数据；5. 延迟/资源/网络审计记录；6. Spike 报告（候选对比、ρ、失败记录、样本量声明、验收逐项对标）；7. 结构化实现报告（修改文件清单、diff、测试/测量结果、失败尝试、下一步建议）；8. 下一窗口唤醒卡。

## 17. 下一张交接卡要求

- Spike 完成后：Builder → 独立 Reviewer（Review 3 代码与行为 Review，核验冻结记录/提交历史/验收逐项/零产品改动）；
