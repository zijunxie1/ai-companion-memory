...
- **GOV-CHIEF-001**：只落盘执行/决策 Chief 边界并校准治理状态，不修改产品；
- **GOV-COMM-001**：只实施已单独批准的沟通与交接规范，不修改产品；
- **GOV-002**：作为候选治理任务单独起草、批准、分支和 Review，不在本次治理同步中顺带实现；
- **本地 Gate Spike**：作为独立、限时、无用户数据外发的技术验证，最多验证两个候选机制，不接入产品路径。

> 2026-08-12 治理顺序澄清：新增 GOV-CHIEF-001 → GOV-COMM-001 作为 TASK-006 内部前置治理，只同步 Founder 已批准的治理顺序。它不改变产品主线 `TASK-005A → TASK-006 → TASK-007 → TASK-005B`，不改变本文件路线 B 的技术结论、TASK-006 状态、E004 验收标准或产品实现授权。
...
## 6. 作品集展示阶段临时例外（2026-08-14，D-T006-SHOWCASE-EXCEPTION）

Founder 2026-08-14 采纳决策 Chief 对选项 A 的裁决，为本文件的「无用户数据外发」建立**作品集展示阶段临时例外**：

1. **展示阶段**（当前作品集原型 / 面试演示阶段）：取消真实查询和候选记忆的外发限制，允许外部大模型（DeepSeek 等已批准模型）处理 Founder 自有数据；
2. **例外边界**：该例外**不适用于未来正式产品或其他用户**；仅针对 Founder 个人作品集展示；
3. **未来产品化**：相关性判断模型优先部署在本机，恢复默认不外发，即回到本文件路线 B 的技术结论；
4. **不变项**：密钥保护、测试、Review、费用记录及合并部署权限不变；
5. **本轮授权边界**：本裁决只授权执行 Chief 落地纯治理同步（本文件 + product.md + decision-register.md + current-state.md + project-mainline-roadmap.md）；**方案 C 的运行授权未包含在内**，需另一次单独审批。

> 本例外不改变路线 B 作为未来正式产品/多用户的目标路线；不改变 TASK-006 状态、E004 验收标准、主线顺序或产品实现授权。
