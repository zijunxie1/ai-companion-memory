...
先说人话（30 秒）：
第三轮实验全部做完，结论很诚实也很关键：三个方案——本地分数阈值、本地重排模型、外部大模型裁判——没有一个能把"该留的"和"该滤的"可靠分开，连效果上限的外部大模型也失败了。这说明"给每条记忆打分再设阈值"这条路在"失眠↔橘猫"这种隐式关联上走不通。你已经拍板转向"带上下文的记忆可用性判断"。现在第三轮的失败证据已经整理成收尾分支，等你审阅后决定是否建 PR、交给独立 Reviewer。
...
禁止执行：推送、建 PR、合并、继续实验、读取 holdout、运行 R4 实验
验收：确认 4 个收尾文件 + 21 份逐字节一致的证据文件无误，明确下一步授权
停止条件：三方案分离边际 -0.2323/-0.3794/-0.5667 均 ≤0.1 已触发候选级停止；C 波动 0.35 判据失效
