...
> **当前状态：S0.3 方案 C 连通检查已通过（模型名已裁决 deepseek-v4-flash），当前进入 S1 冻结统一候选池与 holdout。**
...
- **已完成**：启动回执；Review 2 实施计划 v1.0 → v1.1；实施计划 v1.1 获 Founder 批准；S0 预装检查完成；方案 B-1 下载核验完成（`model-facts.md`）；S0.3 方案 C 连通检查通过（`preflight-check.md` §S0.3）；模型名已裁决 `deepseek-v4-flash`；密钥特征已清除；**S1 候选池冻结完成**（`candidate-pool-definition.json` + `candidate-pool-freeze.md`）；
- **S1 冻结结果**：候选池 32 对（明确相关 11 / 明确无关 11 / 容易混淆 10 / 零条场景 3 / 关键记忆 5），标签 relevant 16 / irrelevant 16；校准部分 22 对 + holdout 部分 10 对；SHA256 = `70994185...3bff4`；提交 `6b1cb17`；
- **三方案状态**：A（零依赖基线，可执行）✅；B-1（权重已下载核验，可执行）✅；C（连通检查通过 + 模型名已裁决 deepseek-v4-flash，可执行）✅；
- **下一步（S1 已通过）**：进入 **S2/S3 校准集核验与标注**（只接触校准部分 22 对，不读取 holdout 10 对）；
- **冻结纪律（Founder 明确）**：S1 已冻结统一测试题，A/B/C 不得提前查看 holdout；
- **未完成**：S2/S3 校准 + S4 主实验 + S4.5 补充实验 + S5 机制冻结 + S6 holdout 一次性运行并清理 + S7 延迟/资源/费用 + S8 网络审计 + S9 报告；
- **Git 状态**：需 commit + push（candidate-pool-freeze.md 提交号回填 + 交接文件更新）；
- **下一窗口**：Builder 本窗口继续（进入 S2/S3 校准集核验与标注）。

## 19. 下一窗口唤醒卡（S1 已冻结 → S2/S3 校准集核验与标注）

先说人话（30 秒）：
统一的测试题已经造好并冻结了——32 道题，覆盖"该留的""该滤的""容易混淆的""该返空的"和"绝不能漏的关键记忆"五类，三种方案共用同一套。下一步是校准：只在其中 22 道题上调参数、确定每个方案的判断阈值，剩下 10 道"留底题"封存起来谁也不许碰，留到最后一次性验证。

直接复制给下一个角色（≤10 行短卡）：

目标角色：Builder（本长期会话继续）
本次唯一目标：进入 S2/S3 校准集核验与标注（只接触校准部分 22 对，不读取 holdout 10 对）
任务与交接文件路径：project-context/tasks/TASK-006/spike-r3/implementation-plan.md（§7 S2/S3）；candidate-pool-definition.json（校准部分 22 对）
分支：feature/task-006-r3-spike（worktree E:/task-006-r3-spike-worktree）
允许执行：S2 三轮真实本地检索采集原始 score；S3 校准集确定各方案阈值（仅校准部分）
禁止执行：读取/使用 holdout 10 对；真实用户数据；超授权外部调用（C 限 DeepSeek 白名单 deepseek-v4-flash、≤100 次/≤10 元）
验收：校准集三轮检索原始数据落盘；各方案阈值仅在校准集确定；holdout 全程未接触
停止条件：校准集分离边际 ≤0.1 或波动>0.1（候选级停止）；冻结失效 → 停并上报
