...
> **当前状态：S1 已完成，进入 S2/S3（校准集核验与标注；只接触校准部分 22 对，不读取 holdout 10 对）。**
...
- **已完成**：启动回执；Review 2 实施计划 v1.0 → v1.1；S0 预装检查；方案 B-1 下载核验；S0.3 方案 C 连通检查；模型名裁决 `deepseek-v4-flash`；S1 候选池冻结（32 对，SHA256 `70994185...`）；**S2/S3 校准完成**（`calibration-result.md`）；**方案 C 校准完成**（Founder 2026-08-14 批准校准阶段运行后执行）；
...
- **方案 C 校准结果（2026-08-14）**：deepseek-v4-flash 批处理（4 场景 × 3 轮 = 12 次调用，失败 1 次 round2-S5 parse_mismatch）；分离边际 **-0.5667**（≤0.1，候选级停止）+ 波动 **0.35**（>0.1，判据失效）；关键门表面 6/6 但为 thr=0.11 全保留假象；LLM 把无关"同事的猫"（S3-05）判 0.7167 高分、把关键隐式关联正例 S1-04（0.30）/S5-05（0.15）判低——**外部大模型同样未能建立"失眠↔橘猫"隐式关联可靠分离**；证据 `data/calibration/scheme-c-result.json`；
- **三方案状态**：A（候选级停止：分离边际 -0.2323）❌；B-1（候选级停止：分离边际 -0.3794 + 独立门漏 3 配对，涉及 K1/K4）❌；C（候选级停止：分离边际 -0.5667 + 波动 0.35）❌；
- **当前阻塞**：三方案均触发候选级停止 → 第三轮 Spike 形成"部分证据"而非"完整对比通过"（DRAFT §8.0）；
- **下一步**：返回 Founder 裁决（三方案均候选级停止后的去向）；**Builder 不得继续调参、不得缩小验收范围、不得进入 S4 主实验、不得读取 holdout**；
- **Git 状态**：已提交并推送（HEAD = origin/feature/task-006-r3-spike @ `fa90634`；本阶段新增 `calibrate-scheme-c.mjs` + `scheme-c-result.json` + `calibration-result.md` 更新）；
- **下一窗口**：Founder（裁决三方案均候选级停止后的去向）。

## 19. 下一窗口唤醒卡（方案 C 校准完成 → Founder 裁决去向）

先说人话（30 秒）：
校准全部做完了，结果很诚实也很关键：三个方案——本地用分数设阈值、本地重排模型、外部大模型裁判——**没有一个能把"该留的"和"该滤的"可靠分开**。连效果上限的外部大模型也失败了：它把"同事养猫"这种无关记忆判成最高分，却把"养猫陪伴失眠""养猫缓解压力"这种关键记忆判成低分，而且同一道题连跑三次分数还会跳。这基本说明：靠"给每条记忆打个分再设一个阈值"这条路，在"失眠↔橘猫"这种隐式关联上走不通。现在需要你拍板下一步方向。

直接复制给下一个角色（≤10 行短卡）：

目标角色：Founder（裁决三方案均候选级停止后的去向）
本次唯一目标：裁决第三轮 Spike 三方案均不达标后是否继续/缩范围/停止
任务与交接文件路径：project-context/tasks/TASK-006/spike-r3/calibration-result.md（§1/§4bis/§7）
分支：feature/task-006-r3-spike（worktree E:/task-006-r3-spike-worktree，@ 新提交号）
允许执行：Founder 裁决
禁止执行：Builder 在裁决前不进入 S4 主实验、不读取 holdout、不继续调参、不缩小验收范围
验收：Founder 明确选择——继续（需指定新方向）/ 缩范围 / 停止本轮
停止条件：A/B/C 分离边际 -0.2323/-0.3794/-0.5667 均 ≤0.1 已触发候选级停止；C 波动 0.35 判据失效
