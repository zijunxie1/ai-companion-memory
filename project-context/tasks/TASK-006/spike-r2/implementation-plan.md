...
│   ├── cleanup-holdout.mjs       # 删除 holdout 种子并核验清零（S7 内执行；qdrant scroll filter + getAll）
...
S7 holdout 一次性运行（seed → run → cleanup；冻结机制 × 冻结 holdout；质量测试，运行后立即清理并核验清零）
S8 延迟/资源测量（S6 后对冻结机制测；场景 = 校准场景或独立性能合成场景，**不读取冻结 holdout**）
S9 网络/数据边界审计
...
→ S6 机制冻结（步骤③，每正式候选）→ S7 holdout 一次性质量运行（步骤④，运行后立即清理）
→ S8 延迟/资源测量（校准场景或独立性能合成场景，不读取冻结 holdout）→ S9 网络审计 → S10 报告
```
...
3. **延迟/资源测量在机制冻结后（S8 在 S7 之后）**：只对冻结机制测（保证测的就是最终提交的机制）；S8 使用校准场景或独立性能合成场景，**不读取冻结 holdout**——冻结 holdout 仅用于 S7 一次性质量测试，运行后立即清理，避免延迟测量重复使用冻结测试场景、破坏一次性运行隔离；测量本身不改变机制。
...
- 每场景使用全新评测专用 user（`eval-spike-r2-h<1-4>-<rand>`，合成种子经 §11.3 零外发路径写入，S7 质量测试运行后立即清理并核验清零）；
...
|---|---|
| 预热 | 机制函数先执行 5 次，结果丢弃 |
| 样本次数 | 每场景有效测量 N ≥ 30（场景 = 校准场景 E001–E005 或独立性能合成场景，**不读取冻结 holdout**；N 与理由写入报告） |
...
3. **运行期审计**：校准评分、holdout 运行、延迟测量全程在包装下执行，审计日志随证据入库（校准/holdout/延迟三阶段 + 清理核验记录）；
...
6. **种子清理核验**：holdout 种子 S7 质量测试运行后立即删除，`qdrant scroll filter user_id` 逐 user 验证 0 剩余（更底层，直接打向量库）+ `mem0.getAll` 复核 + cleanup.json 记录；
7. **零产品改动合规证据**：`docker diff v2-mem0-server` 输出作为"未改产品代码"证据（应仅 A=新增，无 C 修改产品文件）。

### 11.3 零外发种子写入（S7 seed-holdout.mjs）与清理（S7 cleanup-holdout.mjs）
...
// 时序：S7 写入种子 → S7 一次性质量测试 → S7 立即清理并核验清零；S8 延迟测量不读取冻结 holdout（用校准/独立性能场景）
```
...
| 5 冻结 holdout 一次性运行 F1 ≥ 0.9 且边际 > 0.1 | 四步顺序执行（S1→S2/3→S6→S7 质量运行）；一次性运行；运行时哈希断言；运行后无继续调参提交（Reviewer 核对提交历史）；运行后立即清理并核验清零（S7 内完成） | holdout-freeze.md、mechanism-freeze.md、data/holdout |
...
S7  holdout 一次性质量运行（步骤④）：seed-holdout（零外发写入）→ run-holdout（运行时哈希断言，exit(2) 保护）→ 质量结果记录 → 立即清理并核验清零（cleanup-holdout：qdrant scroll + getAll）；运行后禁止继续调参
S8  延迟/资源测量（冻结机制；场景 = 校准场景 E001–E005 或独立性能合成场景，**不读取冻结 holdout**）：预热 5、N≥30、基线/处理组/增量、P95 nearest-rank、RSS/CPU → data/latency
S9  网络/数据边界审计 → data/audit（校准/holdout/延迟三阶段 + 清理核验记录）
...
- 提交策略：每阶段（S0/S0.5/S1/S3/S6/S7/S8/S10）一个证据 commit，commit message 标注阶段与冻结事实（时间戳/哈希）；
