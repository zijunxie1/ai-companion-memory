...
**普通交接（L2）只显示**：先说人话（30 秒）+ 一张可复制短卡；禁止附加重复状态报告、技术附录或重复解释"接下来会发生什么"。
...
**Reviewer 交接规则**：独立 Review 是完成和合并前的**必经步骤**；Founder 决定**何时**把交接卡发送给 Reviewer 窗口，而不是决定**是否需要** Review。Builder 完成规则修改后停在 IMPLEMENTED，不得自行宣布 Review 完成。

## 4. 下一窗口唤醒卡（发送包装说明）

> **本文件 §3.1 的十七字段交接卡是唯一正式交接卡结构**；本节只是"发给 Founder 的发送包装说明"，**不是另一套简版交接卡**。Founder 复制发送的主体必须是**完整十七字段交接卡**（按 §3.1 填写，模板见 `project-context/templates/role-handoff-template.md`），再附本节包装说明指明目标角色与必读文件。
...
目标角色 / 会话：
为什么现在唤醒：
发送主体：完整十七字段交接卡（见 role-wakeup-and-handoff.md §3.1，模板
  templates/role-handoff-template.md），已附于上方"直接复制给下一个角色"；
  本节仅指明发送对象与必读文件。
必读文件（随交接卡一并发送给下一角色）：
  - AGENTS.md
...
请复制发送：
> 你现在是……。如果这是新任或新窗口 Chief，先读取 `project-context/CHIEF-BOOTSTRAP.md`，声明自己是执行 Chief 还是决策 Chief，以及与前任会话的关系（继任 / 角色拆分）。然后从磁盘读取以上文件并做只读核对；回复先写“先说人话（30 秒）”，再在启动回执中报告 AGENTS 规则版本、分支、Worktree 和工作区状态。确认无冲突后，只执行……。不要……。遇到八类升级事项时形成升级卡，不自行收敛。完成或阻塞时更新交接文件，并返回下一窗口唤醒卡。
```
