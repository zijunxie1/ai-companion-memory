...
这是**可以准备交给 Builder 的内容门**，不是当前窗口自动获得写文件、提交、推送、建 PR、运行、下载、外部调用或唤醒角色权限的证明。实际动作必须以当前 Founder 指令或交接文件中的“允许执行 / 禁止执行”为准；既有只读或禁止项在状态变化后继续有效，直到被明确解除。

任务状态变化与治理落盘也必须分开：状态可以在聊天中被 Founder 决定，但只有本窗口已获写入权限时，Agent 才能修改任务文件、`current-state.md`、`decision-register.md` 或创建 Git 提交；否则记录为“已决定，待授权落盘”。
...
2. **Founder 聊天（简短结果）**：3—5 句小白能懂的话说明现在处于什么位置、能否继续、继续的实际影响、真正风险、建议和 Founder 只需决定什么；给出完整报告的 PR 位置。若下一步仍待 Founder 决定，不附短卡；Founder 明确选择后才直接给 ≤10 行短卡。
