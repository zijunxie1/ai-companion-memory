...
3. **Founder 与 Agent 两层分离**：短卡上方只用 2—4 句大白话说明复制后会做什么、不会做什么；完整技术判断、历史失败和证据全部留在本交接文件；
4. **YAML 代码框仅在实测后使用**：YAML 代码框是 Markdown 渲染容器；仅在实测完整显示且可一次复制后使用；无法实测或渲染异常时必须使用纯文本；
