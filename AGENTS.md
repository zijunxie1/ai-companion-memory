...
- **两类受众分离**：给 Founder 的正文以听懂和判断为目标；给后续 Agent 的交接以完整、准确执行为目标。技术执行上下文不得倾倒给 Founder，也不得为简化 Founder 回复而从交接文件丢失；
- **先讨论，后交接**：讨论阶段禁止提前展示执行卡、任务化或唤醒下一角色；Founder 表示“没听懂 / 先别做 / 未对齐 / 理解偏了”时立即停止推进并重新解释；
- **明确决定直接执行**：Founder 的选择可唯一解释时，直接进入执行或输出交接卡，不再要求二次确认、不发送“决定回执”；只有真实歧义时才用一句话澄清；
- **先等当前决定，再给下一张卡**：Reviewer 报告“可以合并”只是建议，不等于 Founder 已决定合并。此时先用小白能懂的话说明“现在能不能继续、继续有什么影响、建议怎么选、只需要决定什么”，不得提前附合并执行卡；Founder 明确选择后才直接给卡；
...
- **Reviewer 两通道交付**：完整审查报告必须提交到 PR 的 Review / Comment，不得把验收矩阵、日志和问题全文倾倒到 Founder 聊天；聊天只保留大白话结论、必须决策的风险、建议和报告位置。若下一步仍待 Founder 决定，先讨论且不出卡；Founder 决定后才给下一张短卡。提交 PR Review / Comment 是审查证据交付，不属于修改被审分支；
...
- Hermes 的全局 Skill、Memory、旧会话搜索结果和压缩摘要只能帮助定位线索，不能代替本仓库正式规则、当前任务、Git 事实或 Founder 批准边界；冲突时以本仓库重新读取和只读核验结果为准；
- 不得把旧会话的最终回复直接当成本次回复模板。尤其不得因为旧回复曾出现启动表、验收矩阵或下一张卡，就跳过本次“Founder 是否已经听懂并作出当前决定”的判断；
- 只服务某个 TASK、PR、提交或一次实验的知识不得沉淀成跨项目全局 Skill；任务结束后应归档这类一次性 Skill，使其退出后续提示注入。可长期保留的 Skill 必须跨任务复用、无项目私有事实、无固定 PR/提交/实验数字，并且不与仓库规则冲突；
- Agent 不得静默创建、扩写或删除全局 Skill。需要写入时必须说明长期复用价值，并经过 Founder 的 Skill 写入批准门。
...
**仅用于 L3 完整汇报**（任务完成、阻塞、需 Founder 决策或重大风险时；见 `project-context/agent-response-protocol.md` §1.3）。每次重要节点输出：
...
**L1 普通回复与 L2 普通交接不附加本固定状态报告**（L1 通常 3—5 句；L2 只含 2—4 句 Founder 说明 + ≤10 行聊天短卡，完整十七字段仅写入交接文件）。
